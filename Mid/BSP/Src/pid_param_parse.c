...
PID_ParamParser_t pid_param_parser;
/**
 * @brief 初始化PID参数解析器
 */
void PID_ParamParser_Init(void) {
    // 1. 空指针保护：避免传入NULL指针导致内存操作崩溃
    if (&pid_param_parser == NULL) return;

    // 2. 清空接收缓冲区：重置用于存储串口/总线接收的原始数据的缓冲区
    //    防止旧数据残留导致解析错误（比如上次解析的残数据干扰本次解析）
    memset(pid_param_parser.recv_buf, 0, sizeof(pid_param_parser.recv_buf));

    // 3. 重置接收长度计数器：记录接收缓冲区中有效数据长度，初始化为0
    pid_param_parser.recv_len = 0;

    // 4. 重置解析器状态机：将解析状态置为“空闲”（初始状态）
    //    状态机（state）用于管理解析流程（如：等待帧头、接收参数、校验帧尾等）
    pid_param_parser.state = PARSE_STATE_IDLE;

    // 5. 清空已解析的PID参数结构体：确保参数初始值为0，避免脏数据
    //    该结构体存储最终解析出的KP/KI/KD/输出限幅等PID核心参数
    memset(&pid_param_parser.parsed_param, 0, sizeof(PID_Param_t));

    #if PID_UART_DMA_ENABLE
    HAL_UARTEx_ReceiveToIdle_DMA(&PID_UART_UART_HANDLE, pid_param_parser.recv_buf, sizeof(pid_param_parser.recv_buf));
    #endif
}
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size) {
    if (huart->Instance == USART1) {
        ...
        switch(event)
        {
            case HAL_UART_RXEVENT_IDLE:   // 空闲中断（一帧数据发完，总线空闲）
                // 处理收到的数据
                // Led_Toggle(LED1);
                pid_param_parser.state = PARSE_STATE_COMPLETE;
                if(PID_ParamParser_Parse(&pid_param_parser) == PARSE_STATE_COMPLETE)
                {
                    // 解析成功，更新PID参数
                    Led_Toggle(LED2);   
                    g_speed_pid.Proportion = pid_param_parser.parsed_param.kp;
                    g_speed_pid.Integral = pid_param_parser.parsed_param.ki;
                    g_speed_pid.Derivative = pid_param_parser.parsed_param.kd;
                }
                else
                {
                    Led_Toggle(LED1);
                ...
                HAL_UARTEx_ReceiveToIdle_DMA(&PID_UART_UART_HANDLE, pid_param_parser.recv_buf, sizeof(pid_param_parser.recv_buf));
                break;
            case HAL_UART_RXEVENT_HT:     // 半传输（DMA收到一半）
                break;
            case HAL_UART_RXEVENT_TC:     // 全传输（DMA收满）
                break;
        }
    }
}
